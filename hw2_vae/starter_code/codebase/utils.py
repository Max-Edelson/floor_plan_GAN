...
def log_normal(x, m, v):
    ...
    log_p = -0.5*(torch.log(2*torch.pi*v) + ((x - m)**2 / v))
...
def log_normal_mixture(z, m, v):
    ...
    # in the batch
    ################################################################################
    #print(f'z: {z.shape}, z.unsqueeze(1): {z.unsqueeze(1).shape}, m: {m.shape}')
    log_probs = log_normal(z.unsqueeze(1), m, v)
   
    # By subtracting the maximum log probability, we avoid getting very large negative values in the exponentiation which can lead to numerical underflow.
    #max_log_probs = torch.max(log_probs, dim=1, keepdim=True).values
    log_prob = log_mean_exp(log_probs, dim=-1)# + max_log_probs.squeeze()
