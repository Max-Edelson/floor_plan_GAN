...
flags.DEFINE_string('vocab_filepattern', '', 'One or more vocabulary files '
                       '(one word per line as "word,count")')
flags.DEFINE_integer('min_count', 5, 'Minimum subtoken count in corpus')
flags.DEFINE_integer('corpus_max_lines', None,
                        'How many lines of corpus to read')
flags.DEFINE_integer('num_iterations', 5, 'Number of iterations')
flags.DEFINE_bool('split_on_newlines', True, 'Break corpus into lines.')
flags.DEFINE_string('additional_chars', "", 'Set special characters to be included in vocab. ex : "~", "/".')
flags.DEFINE_integer('max_subtoken_length', None, 'Max subtoken length')
flags.DEFINE_bool('backward', False, 'Builds subwords from backward.')
flags.DEFINE_string('log_level', 'INFO', 'Set verbosity of logger')
FLAGS = flags.FLAGS
